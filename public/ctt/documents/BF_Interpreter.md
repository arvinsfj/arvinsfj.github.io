...
程序是有地方保存了，但是程序的产生的中间数据和最终结果保存在哪呢？其实就是上面提到的“the pointer”指向的数组。BF程序是操作这个数组中的数据，比如：读写数据、移动指针和自加自减数据。我们需要一个大小是30000的数组，保存类型暂定为char类型，名称ds。一样的道理需要一个当前位置的标志，名称dp。
